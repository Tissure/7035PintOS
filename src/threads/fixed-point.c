...
int convert_fp_to_integer(int fpNumber)
{
 ...
 //CONVERT_TO_INT_NEAREST(x) (x >= 0 ? ((x + (1 << (FRACTION - 1))) >> FRACTION) : ((x - (1 << (FRACTION - 1))) >> FRACTION))
...
int multiply_fp_by_int(int fpNumOne, int fpNumTwo)
{
    return ((int64_t)fpNumOne) * fpNumTwo / F;
}

int multiply_fp(int fpNumOne, int intNum)
{
    return (fpNumOne * intNum);
}

int divide_fp_by_int(int fpNumOne, int fpNumTwo)
{
    return ((int64_t)fpNumOne) * F / fpNumTwo;
}

int divide_fp(int fpNumOne, int intNum)
{
    return (fpNumOne / intNum);
}
